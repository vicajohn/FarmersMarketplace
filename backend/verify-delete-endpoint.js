...
try {
  ...
  const waitlistRoutes = routes.filter(route => 
    route.path.includes('waitlist') || route.path.includes(':id/waitlist')
  );
  ...
  const deleteWaitlistRoute = waitlistRoutes.find(route => 
    route.methods.includes('delete') && route.path.includes('waitlist')
  );
