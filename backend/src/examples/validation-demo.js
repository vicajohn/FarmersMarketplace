...
async function demonstrateValidation() {
  ...
  for (const input of invalidInputs) {
    ...
    console.log(`   ${input.description}: ${result.success ? 'PASSED' : 'REJECTED'} - ${result.error || 'OK'}`);
  }
  ...
  for (const scenario of scenarios) {
    ...
    console.log(`   ${scenario.name}: ${success ? 'PASSED' : 'FAILED'} - Expected: ${scenario.expected}, Got: ${result.code || 'NO_CODE'}`);
    if (!success) {
  ...
  mockDb.query
    ...
    .mockResolvedValueOnce({ rows: [{ id: 1, name: 'Premium Organic Tomatoes', quantity: 15, is_active: true }] });
  ...
  mockDb.query
    .mockResolvedValueOnce({ rows: [{ id: 123, role: 'buyer', is_active: true }] })
    .mockResolvedValueOnce({ rows: [{ id: 1, name: 'Test Product', quantity: 0, is_active: true }] })
    .mockResolvedValueOnce({ rows: [{ id: 1, position: 5, created_at: '2024-03-15T10:30:00.000Z' }] });
